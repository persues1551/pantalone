...
| 利润分配 | 行业利润率分层 + 集中度趋势 | StockAnalysis 行业对比 |
...
| 行业利润断层(I) | 营业利润率 > 同行中位数 + 竞争优势持续期 | SEC 10-K/10-Q financials |
| 财务三爆(F) | 营收>15% + FCF yield>3% + ROE>15% | 10-K/10-Q + StockAnalysis cash flow |
...
| A股框架 | 美股映射 | 工具 |
|---------|----------|------|
| 技术指标 | MA/RSI/MACD/布林带（query1 chart API） | query1.finance.yahoo.com/v8/finance/chart |
| 资金流向 | ETF申赎、put/call ratio、margin debt | FINRA、CFTC |
| 宏观因子 | 利率敏感度、美元强弱、信用利差 | FRED数据 |
| 风险收益比 | 历史VaR、ATR、最大回撤 | query1 chart API + 自行计算 |
...
| 实时数据 | 腾讯API/AKShare（免费） | query1 chart API（免费，延迟15分钟） |
...
> 2026-08-13 实测修正：**yfinance 的 `info`/`financials`/`download` 在大陆网络不可用**（fc.yahoo.com 被 TLS 干扰，且 1.5.x 强制 curl_cffi 在 macOS 上有 TLS 缺陷）。K线改用 `query1.finance.yahoo.com/v8/finance/chart/` 直连（已验证 HTTP 200）；财务和评级用 SEC EDGAR + StockAnalysis.com。完整实测记录见 `references/institutional-research-sources.md`。

| 优先级 | 数据源 | 适用范围 | 限制 |
|--------|--------|----------|------|
| 1 | SEC EDGAR | 10-K/10-Q、8-K、proxy | 免费、权威、10次/秒限流 |
| 2 | query1 chart API | K线/历史行情 | 免费，`query1.finance.yahoo.com/v8/finance/chart/{SYMBOL}`，无 cookie 需求 |
| 3 | StockAnalysis.com | 分析师共识、目标价、评级计数 | 免费、可解析、聚合观点无券商名 |
| 4 | Financial Modeling Prep | 结构化财务、DCF、评级 | 250次/天免费、需API key |
| 5 | 券商研报（A股东财接口/美股聚合） | 行业分析、目标价 | 可能有利益冲突，作为参考；A股接口见 `references/institutional-research-sources.md` |
| 6 | TradingView/Finviz | 技术图表、screener | 辅助工具，不直接引用 |

## 失效条件

美股分析在以下条件失效时必须标注：

1. query1 chart API 连续失败 → 跳过实时数据，使用最近缓存并标注
2. SEC EDGAR CIK找不到 → 降级为 StockAnalysis 数据
3. 非交易日 → 使用最近交易日数据，标注日期
4. ADR币种不匹配 → 标注原始币种，不做自动换算
5. 财报口径不一致 → 标注GAAP/non-GAAP差异
6. yfinance 的 info 接口不可用（大陆网络）→ 不降级为"无数据"，改用 EDGAR + StockAnalysis + query1 chart（见数据源等级）
