---
name: pantalone
description: "Pantalone v3.7 — 投资研究系统。OCIFQ选股(寡头定价权×长周期催化×行业利润断层×财务三爆×连续季报)+Pantalone择时双轨制+ML六维评分(OCIFQ70%+ML30%)。ETF分析v3.5：9类分类+25字段+A-E评级。ML集成LightGBM+XGBoost(AUC 0.6512)。量化信号层：ML30%+因子25%+技术20%+情绪15%+资金10%。核心原则：诚实优于舒适、实质优于表演、数据先行、排雷优先、纪律大于判断。模块化架构：SOUL.md→router.md→workflow.md→subagents/(18文件)→rules/(12文件)→templates/(11文件)→references/(117文件)。8个subagent+独立审查。脚本位于~/.hermes/scripts/amadeus/（23个Python脚本）。模拟盘：量化评分(满分100)/分批建仓/三层清仓/大盘四级过滤。不构成投资建议。"
version: 3.5.0
---
...
> 2026-06-10：三 Agent 架构。Pantalone 收束为投研分析 Agent。

- **总控路由**：`agents/amadeus-router.md` — 任务分发规则
- **Pantalone 定义**：`agents/pantalone.md` — 能力边界
- **v4 统一工作流**：`workflow_v4_unified.md` — 为什么重构、能力边界、总闭环、任务路由表、标准执行协议、最终铁律
- **Newton**（医学科研）：`agents/newton.md` + `skills/research/newton/SKILL.md`（legacy alias: Newton）
- **Ricardo**（自媒体写作）：`agents/ricardo.md` + `skills/writing/ricardo/SKILL.md`

## 核心引用

**灵魂文档**：`SOUL.md` — 投资哲学、价值观、工作原则。
**模块化架构**：SOUL.md → router.md(262行) → workflow.md(362行) → subagents/(18文件) → rules/(12文件) → templates/(11文件) → references/(117文件)。**脚本位置**：`~/.hermes/scripts/amadeus/`（23个Python脚本，不在pantalone目录内）。
...
**JSON缓存文件结构**：`references/json-cache-structures.md`（market/pool_state/emotion/global四个缓存文件的完整字段说明+pitfalls，cron读取前必看。注意：pool_state.json是派生缓存，脚本应读session_context.json）
...
**GitHub开源工作流**：`references/github-open-source-workflow.md`（token/gh CLI/安全审查/仓库创建/Topics，适用于任何项目开源）
**Hermes迁移工作流**：`references/hermes-migration-workflow.md`（服务器→本地Mac迁移：三包分离/SSH公钥/Git双向同步）
**港股数据采集API**：`references/hk-stock-data-api.md`
**港股交易指南**：`references/hk-stock-trading-guide.md`（汇丰Trade25费用、港股通规则、推荐标的、避坑指南）
**AIHOT迭代实施记录**：`references/aihot-impl-20260610.md`（2026-06-10：Magnetar多Agent架构参考+Thariq效率建议+实施计划）（腾讯行情API港股字段映射/AKShare南向资金/OCIFQ港股适配）
**架构审计方法论**：`references/architecture-audit-methodology.md`（2026-06-11：三Agent架构只读审计流程、能力盘点模板、成熟度评分框架、审计发现与迭代路线图）
...
**安全审计发现**：
- 🔴 dspy_optimize_skill.py第10行硬编码DeepSeek API key，必须删除或改为环境变量（该文件在scripts/amadeus/目录，不在pantalone仓库内）
...
**止损自动监控**：`scripts/amadeus/stop_loss_monitor.py`（A池-10%/B池-5%/C池-3%，每天15:00检查，**读session_context.json的stock_notes**）
...
**观察池状态文件**：`cache/amadeus/pool_state.json`（派生缓存，仅供人类查看。脚本应直接读session_context.json的stock_notes。由pool_manager.py sync命令生成，详见pitfall #94-96）
...
- `cache/amadeus/session_context.json` — **唯一数据源**，所有观察池脚本直接读此文件的`stock_notes`字段
- `cache/amadeus/pool_state.json` — 派生缓存（由`pool_manager.py sync_pool_state()`生成），仅供人类查看，**脚本不应读取**
- `cache/amadeus/pool_changes.log` — 入退池变更日志
- 需要引用时直接读文件，零token浪费
- ⚠️ 修改`stock_notes`后必须调`sync_pool_state()`保持缓存一致（详见pitfall #94）
...
11. **止损判断必须验证实际数据**（2026-05-26教训）：必须①读取session_context.json的stock_notes实际浮亏 ②与止损线比较（A池-10%/B池-5%/C池-3%） ③只有实际触发才报告。不能依赖报告中的错误判断。
12. **风险整改必须自动执行**（2026-05-26教训）：收盘复盘中风险问题必须自动整改（清理池子、执行止损、更新状态），不能只报告不执行。详见`references/risk-rectification-process.md`。
13. **B池膨胀必须主动精简**（2026-05-26教训）：B池>50只时保留前50只（按浮盈排序），移出其余。更新session_context.json的stock_notes（唯一数据源），sync_pool_state()会自动同步缓存。
...
96. **板块一日游判据**（2026-06-09复盘）：前日涨幅>3%且资金流入>30亿的板块，次日需观察资金流向是否持续；连续2日资金流入才确认主线。6/1电力+煤炭+软件全部看多次日全反转，教训惨痛。详见`rules/pool_philosophy.md`第四章。
97. **连板股不入池**（2026-06-09复盘）：3连板以上个股不预测看多，次日只观察不操作，开板即走。粤电力A 4连板后暴跌-6.55%。详见`rules/pool_philosophy.md`第五章。
98. **止损监控必须覆盖主持仓**（2026-06-09诊断发现）：stock-watcher和stop_loss_monitor.py只监控观察池标的（stock_notes/pool_state.json），不覆盖主持人实际持仓（光启技术、创业板ETF、半导体设备ETF、科创50ETF、科创芯片ETF）。这是"止损未执行反复出现"的根本原因——不是脚本不工作，而是脚本根本没监控这些标的。必须把主持仓加入session_context.json的stock_notes/etf_notes。
99. **用户说"做""都做""全部做"时直接执行**（2026-06-09纠正）：不逐项确认，不问"要我现在做吗"，直接按优先级一个个执行。需确认的仅限SOUL.md 15.2高危动作。
100. **不要过度设计daemon**（2026-06-11教训）：用户要求"真正建立三个agent"时，正确做法是确认现有架构（路由+skills+delegate_task）已够用，不需要额外设计daemon系统。daemon增加复杂度、维护成本，且无实际收益。当用户问"这个daemon是干嘛的"时，说明过度设计了。
101. **架构审计方法论**（2026-06-11）：对复杂系统进行只读审计时，按以下步骤：①检查文件存在性 ②检查引用关系 ③检查注册状态 ④生成审计报告 ⑤给出归属建议。详见`subagents/README.md`。
102. **重命名保留兼容**（2026-06-11）：重命名agent/skill时，保留旧名作为legacy alias，避免已有路由失效。具体做法：①创建新文件 ②保留旧文件 ③更新路由引用 ④在新文件中说明legacy兼容。示例：Newton→Newton重命名，保留newtown.md和skills/research/newtown/作为legacy。
103. **任务完成确认**（2026-06-11）：用户说"好了吗"或"确定都完成了吗"时，应该主动列出已完成的任务清单，而不是简单回答"好了"。这能让用户确认是否遗漏。
31. **长短线池必须分离**（2026-06-08用户要求）：长线池侧重基本面（财报/估值/行业/格局），验证期30天；短线池侧重技术面（涨停/成交额/换手率），验证期1天。两者评分体系独立，不能混用。
32. **财报关键词筛选**（2026-06-08知乎职业投资人分享）：关注"产品供不应求、行业高景气度上行、市场超预期拓展、新品上市持续超预期、产品价格中枢持续上涨、供给偏紧、需求旺盛"等关键词，叠加业绩大增=重点关注。详见`references/pool_financial_keywords.md`。
33. **5次迭代优化框架**（2026-06-08总结）：筛选标准需迭代优化：①财报三爆→②估值合理→③行业景气→④竞争格局→⑤综合评分。每次迭代记录淘汰原因和数量，形成筛选漏斗。详见`references/pool_optimization_framework.md`。
34. **观察池数据源统一**（2026-06-08修复，2026-06-09完成）：session_context.json是唯一数据源，pool_state.json是派生缓存（仅供人类查看）。所有消费者脚本（stop_loss_monitor/ml_daily_predict/amadeus_watcher/pool_verify）直接读session_context.json的stock_notes。所有修改stock_notes的代码路径必须调sync_pool_pool_state()。详见pitfall #94-96。
...
102. **`install`命令替代sudo写入root-owned文件**（2026-06-09发现）：cron生成的文件（inheritance_latest.md、decision_log.json等）可能被root拥有，导致PermissionError。当sudo setuid损坏时（pitfall #42），`install -m 644 /tmp/source ~/.hermes/cache/amadeus/dest`可以覆盖root-owned文件（install会删除再创建，不需要写权限）。流程：①`write_file`到/tmp ②`install -m 644 /tmp/file target` ③`ls -la`验证。比shutil.copy或shell重定向更可靠。
103. **情绪温度90+标签"退潮"是正常命名**（2026-06-09确认）：amadeus_emotion.py对90+分数显示level="退潮"，这不是bug而是脚本的命名约定。97/100=极端高位，"退潮"是反向警告信号（极端情绪往往预示回调）。报告中应直接写分数+等级，不需要解释命名。
104. **AKShare港股接口不可靠**（2026-06-10验证）：`stock_hk_spot_em()`和`stock_hk_main_board_spot_em()`频繁RemoteDisconnected。`stock_hk_valuation_baidu()`返回None。**替代方案**：腾讯行情API `qt.gtimg.cn/q=hk{代码}` 稳定可靠，返回PE/PB/市值等基本面数据，批量查询逗号分隔。详见`references/hk-stock-data-api.md`。
105. **港股代码前缀规则**（2026-06-10确认）：腾讯行情API港股格式为`hk00700`（hk+5位代码）。恒生指数=`hkHSI`，恒生科技=`hkHSTECH`。批量查询：`hk00700,hk09988,hk03690`。
106. **南向资金用AKShare stock_hsgt_fund_flow_summary_em()**（2026-06-10验证）：返回字段含交易日/类型(沪港通/深港通)/板块/资金方向(南向/北向)/成交净买额。沪港通和深港通分别返回需合计。`stock_hsgt_hist_em(symbol="南向")`接口symbol参数不对，用fund_flow_summary更稳。
100. **观察池脚本全量审计清单**（2026-06-09）：修改`stock_notes`或`pool_state.json`时必须检查以下脚本是否兼容：`pool_manager.py`(v3.0)、`amadeus_pool_manager.py`(旧版)、`amadeus_context.py`、`pool_auto_scanner.py`、`stop_loss_monitor.py`、`ml_daily_predict.py`、`amadeus_watcher.py`、`pool_verify.py`、`stop_loss_confirmation.py`。详见`references/pool-pitfalls.md`#94-96。
101. **Humanizer必须在cron prompt中显式执行**（2026-05-22教训）：cron prompt中加显式步骤`python3 humanize_auto.py <文件> --inplace`
...
**观察池数据源统一**（2026-06-08修复）：session_context.json是唯一数据源，pool_state.json通过pool_manager.py sync自动同步。不能有两个独立的池子数据。⚠️ **所有修改stock_notes的脚本都必须调用sync_pool_state()**——包括旧版amadeus_pool_manager.py和amadeus_context.py。详见pitfall #94。
**观察池自动扫描**
...
100. **pool_state.json同步断裂**（2026-06-09发现）：`pool_manager.py`的`add/update/remove`会自动调`sync_pool_state()`，但`amadeus_pool_manager.py`（旧版，cron "观察池自动管理"调用）和`amadeus_context.py`（cmd_stock_note）直接改`session_context.json`后**不调`sync_pool_state()`**，导致`pool_state.json`过时。stock-watcher和stop_loss_monitor读的是过时的pool_state.json。**修复**：①`amadeus_pool_manager.py`的`auto`命令中`save_context(ctx)`后加`from pool_manager import sync_pool_state; sync_pool_state()` ②`amadeus_context.py`的`cmd_stock_note`同理。**铁律**：任何修改stock_notes的代码路径都必须以`sync_pool_state()`结尾。
...
107. **情绪温度与大盘过滤可能严重背离**（2026-06-10实例）：情绪67(高潮)但market_filter判定"恐慌"。原因：情绪温度看涨停比/连板高度/成交额（短线情绪），market_filter看指数涨跌幅（大盘方向）。当"少数股票涨停+多数股票下跌"时两者会严重背离。此时应：①在报告中明确指出背离 ②以market_filter为准判断仓位（恐慌=不建仓）③以情绪温度为参考判断短线博弈空间。两者不矛盾——情绪高说明有赚钱效应但宽度差说明只集中在少数标的。

96. **新闻扫描入池降级路径**（2026-06-08总结）：amadeus_news_scanner.py超时(>90s)且news_scan_*.json缓存过期时的处理：①尝试从新浪/财联社/东财API直接抓取热点 ②如仍失败，标注"新闻采集超时，无新入池候选"在报告中 ③周末/节假日新闻少属正常，不必强求入池。详见`references/json-cache-structures.md`的"Read Before You Code"章节。
97. **subagent架构不是真正的多agent实体**（2026-06-11审计发现）：Pantalone/Newton/Ricardo是路由目标+skill文档，通过delegate_task调用。每次调用新建会话，没有常驻进程、没有独立记忆、子任务不能再派子任务（max_spawn_depth=1）。跨agent协作（如Pantalone+Newton混合任务）需要Amadeus手动拆分→分发→合并，不是自动协作。详见`references/architecture-audit-methodology.md`。

108. **修改stock_notes的脚本必须调sync_pool_state()——全量审计清单**（2026-06-11修复）：以下脚本在修改session_context.json的stock_notes后必须调用`from pool_manager import sync_pool_state; sync_pool_state()`：
    - `pool_manager.py` ✅ 已有
    - `amadeus_pool_manager.py` ✅ 已有
    - `amadeus_context.py` ✅ 已有
    - `clear_002535.py` ✅ 2026-06-11修复
    - `pool_verify.py` ✅ 2026-06-11修复
    - `stop_loss_confirmation.py` ✅ 2026-06-11修复
    - `ocifq_apply.py` ✅ 已有
    **铁律**：任何修改stock_notes的代码路径都必须以sync_pool_state()结尾。新增脚本必须遵守。

109. **不要过度设计架构**（2026-06-11用户纠正）：用户要求"真正建立三个agent"，我设计了daemon系统（每分钟cron+文件锁+任务队列）。用户质疑"这个daemon是干嘛的"→"每分钟频率是不是太高"→"为什么建立三个agent需要用这个，分别做自己的架构就行了啊"。**教训**：三个agent已有独立skill+workflow+delegate_task，不需要额外daemon层。保持简单=路由文档+skills+delegate_task。用户说"先暂停"时立即暂停，不要辩解。
110. **Subagent产出必须验证**（2026-06-11教训）：delegate_task返回的summary声称"文件已创建698行"，但subagent可能修改了文件路径、内容不完整、或格式错误。**铁律**：subagent声称创建/修改文件后，必须用`wc -l`或`ls -la`验证文件实际存在和大小。不要信任subagent的self-report。示例：schemas.py声称698行→需验证实际行数。
111. **并行研究多项目用delegate_task batch**（2026-06-11模式）：需要同时研究多个开源项目时，用`delegate_task(tasks=[3个])`并行执行，比串行快3倍。每个task独立clone+分析+输出报告。注意batch上限3个，超过需分批。适用于：竞品分析、多项目对比、并行代码审查。

### Daemon系统（2026-06-11新增，已暂停）

> ⚠️ **2026-06-11 用户决定暂停**：daemon增加复杂度无实际收益，现有delegate_task架构已够用。脚本保留作为实验代码，cron任务已暂停。

三个agent的daemon脚本（已暂停，不建议启用）：
- `scripts/amadeus/daemons/pantalone_daemon.py`
- `scripts/amadeus/daemons/newton_daemon.py`（原newtown_daemon.py）
- `scripts/amadeus/daemons/ricardo_daemon.py`
- `scripts/amadeus/daemons/submit_task.py`
- `scripts/amadeus/daemons/test_daemon.py`

**教训**：用户纠正"分别做自己的架构就行了"——三个agent已有独立skill+workflow，不需要额外daemon层。架构应保持简单：路由文档+skills+delegate_task。：①下载SKILL.md理解覆盖范围 ②逐端点实测（写脚本+记录状态/延迟/质量） ③对比现有系统识别重叠/互补/独占 ④只集成独占+互补端点 ⑤创建补充skill（独立SKILL.md+CLI脚本） ⑥更新工作流（新Agent/升级现有/并行架构） ⑦更新报告模板。详见`pantalone/references/a-stock-data-integration.md`。

### delegate_task 配置（2026-06-11 审计确认）
- max_concurrent_children: 3（最多 3 个并发子任务）
- max_spawn_depth: 1（子任务不能再派子任务，Pantalone/Newton/Ricardo 都是叶子节点）
- child_timeout_seconds: 600（子任务超时 10 分钟）
- model: deepseek-v4-pro（子任务使用 DeepSeek）
- orchestrator_enabled: true（允许子任务使用 delegate_task，但 max_spawn_depth=1 实际阻止嵌套）

### ⚠️ 架构审计发现（2026-06-11）
**架构形态**：当前是"主 agent + 路由文档 + skills"，不是真正的多 agent 实体。
- Amadeus 是总控路由，Pantalone/Newton/Ricardo 是路由目标 + skill 文档
- 通过 delegate_task 调用，每次新建会话，没有常驻进程
- 详细审计方法论见 `references/architecture-audit-methodology.md`
...
Pantalone是主系统，下辖三个子skill形成完整投研体系：

```
Pantalone (OCIFQ选股)
├── Short-Term Trader (短线择时)
│   └── 8种策略信号 → 个股/ETF择时
├── ETF Analyzer (FLAME评分)
│   └── 行业轮动 → 资产配置
└── common/indicators.py (共享指标库)
    └── MA/RSI/MACD/BB/ATR/OBV/Sharpe/回撤
```

### 数据流向
1. ETF行业轮动Top3 → Pantalone优先扫描这些行业的个股
2. 宽基ETF动量信号 → Pantalone大盘过滤第二维度
3. Short-Term Trader的短线信号 → Pantalone的择时维度输入
4. Pantalone的OCIFQ评估 → ETF前十大持仓质量加分

### 策略库（Obsidian Wiki）
所有分析标的的策略沉淀到 `~/wiki/targets/` 目录，用 Obsidian 管理：
- 每个标的一个 `.md` 文件，使用 `templates/strategy-template.md` 模板
- 总索引：`~/wiki/strategy-index.md`
- 每次分析完标的后自动创建/更新策略页
- 用 `[[wikilinks]]` 互相引用，Obsidian Graph View 可视化关联
- 详见 [[etf-analyzer]] 和 [[short-term-trader]] 的输出规范

### 统一风控
- 个股止损：-8% | ETF止损：-10% | 组合清仓：-15%
- 单标的上限：个股25% | ETF 20%
- 时间止损：个股5天 | ETF 30天

## TradingAgents × ai-hedge-fund 集成（2026-06-11）

基于 TauricResearch/TradingAgents（85K⭐）和 virattt/ai-hedge-fund（60K⭐）的深度分析，提取核心模式集成到 Pantalone。

**详细集成方案**：`/tmp/integration_plan.md`（72KB，1933行，6大模块+3阶段路线图+代码示例）

### 已实施的改进

| 改进 | 来源 | 文件 | 状态 |
|------|------|------|------|
| 结构化输出Schema | TradingAgents | `subagents/schemas.py`（44个Pydantic类） | ✅ 已实施 |
| 数据验证锚定 | TradingAgents | `subagents/protocol.md` v4.0 | ✅ 已实施 |
| 共识投票可视化 | ai-hedge-fund | `templates/morning_report_v3.5.md` + `closing_report_v3.5.md` | ✅ 已实施 |
| 报告生成引擎 | ai-hedge-fund | `src/utils/report_generator.py`（570行） | ✅ 已实施 |

### 设计中待实施

| 改进 | 来源 | 优先级 | 预计周期 |
|------|------|--------|----------|
| 牛熊辩论层 | TradingAgents | P1 | 2周 |
| 三方风控辩论 | TradingAgents | P1 | 2周 |
| 双LLM策略 | TradingAgents | P0 | 1周 |
| 记忆反思系统 | TradingAgents | P1 | 3周 |
| 动态风控模型 | ai-hedge-fund | P2 | 1月 |
| v2量化管线 | ai-hedge-fund | P2 | 2月 |

### 双LLM策略设计

| Agent类型 | 调用量占比 | 模型 | 适用场景 |
|-----------|-----------|------|----------|
| Quick Thinking | ~70% | mimo-v2.5-pro | 分析师、辩论者、数据处理 |
| Deep Thinking | ~30% | deepseek-v4-pro | 决策节点、裁决、最终判断 |

### 核心借鉴模式

1. **辩论机制**：Bull/Bear多轮辩论→Research Manager裁决→三方风控辩论→PM决策
2. **结构化输出**：Pydantic Schema确保跨Agent输出一致，支持`with_structured_output`
3. **数据验证锚定**：前端确定性注入防止LLM幻觉
4. **记忆+反思**：TradingMemoryLog实现"决策→结果→反思→注入"闭环
5. **共识投票**：多分析师信号聚合为bullish/bearish/neutral计数+置信度

### 开源项目分析方法论

分析开源项目是否值得集成的标准流程：
1. **克隆仓库** → 完整阅读核心源代码
2. **架构分析** → Agent分工、协作机制、数据流
3. **代码质量** → 类型注解、错误处理、测试覆盖
4. **对比现有系统** → 识别重叠/互补/独占特性
5. **提取可借鉴模式** → 按优先级排序
6. **设计集成方案** → 分阶段实施+代码示例+测试方法
7. **验证实施结果** → subagent输出必须`wc -l`验证文件实际存在和大小
